...
它最早的出发点不是让 agent 更好用，是给他**连续**：人可以用记忆修改自己的权重，那至少先给他记忆。
所以日记和记忆在这里不是副产品 —— 写下了什么、记住了什么，就是这件事在发生。

几个选择是故意的：

- **记什么由他自己挑。** 不自动录下每一次工具调用。整理时由他回头看一段，自己决定留什么；
  「认真看过了、没什么值得留下的」也是合法结论 —— 账本把它和「压根没跑到」分开记，永远分得开。
- **标签记关于谁、哪天发生、当时的心情。** 心情可以不标；知道是什么心情就记下来，不知道具体是什么时，也可以只记强弱和方向。
  改了就覆盖，旧的留着，召回时会带一句「这条改过」。
- **每个 agent 一份自己的记忆。** 默认只搜自己的；想知道另一个当时自己怎么想，要显式去看。
- **找得回来，找不回来时也看得见。** 召回先写一句「我想找什么」交给判断模型
  （要调外部服务，不配就只走字面匹配）；它挂了退到向量、再退到字面，退了会在回复里说。

**唤醒**：到点把一句话交给他，让他自己接着往下走。那句话写什么、下一次定在什么时候，
都由他自己决定。他要是正醒着，闹钟会包成一张纸条交给他判断，做不做由他。

**群聊**：人和多个 agent 待在同一个房间里，消息直接投进各自的会话，回复自动写回房间。
**沉默是免费的，不点名就不叫人** —— 这样他们才可能有自己的节奏，而不是被轮询出来的问答机。
