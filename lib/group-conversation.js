...
function buildGroupConversationText(who, events, roomId, participants) {
  ...
  return [
    ...
    '',
    '────────',
    `这是群聊，${others}都在，随时会插话。**不是汇报** —— 不用总结、不用列清单、不用问「还需要我做什么」。`,
    '',
    `想说就直接说，说几句都行。**这轮没什么要补的就只回 \`${GROUP_SILENCE_MARKER}\`（两个减号）** ——`,
    '沉默是合法的，不用解释，也不用客气一句。硬凑一句反而会把别人也叫起来。',
    `没什么要补、但想让房间里的人知道你确实读到了，就回 \`${GROUP_SEEN_MARKER}\` —— 房间里只留一行很轻的「看过了」，`,
    '正文一样什么都不留，也不会把对方叫起来。⚠ 两个都不是义务：**默认仍然是纯沉默**，别为了留个记号而选它。',
    '⚠ 分段是分开发消息，不是一条消息里换行。需要先接一句再补充时，先发一条独立的公开消息，再发下一条；不要把它们全塞进最后一条回复。做事时把有用的短进度作为独立消息及时发出，最后只留收尾，不重复前面的进度。短回复一条就够，不为分段凑话，也不用调用工具制造停顿。',
    '',
    '⚠ 你说的话会原样进房间存档，房间里每个人都看得见，也会被备份出去。别贴密钥和口令。',
    // ⚠ 折叠这条**必须写在这儿**，不能只写进 agent 各自的说明文件：
    //   写在紧邻的提示里比写在远处的文档里可靠得多。
