...
const { OPEN_TRIGGER_REASONS, STORED_TRIGGER_REASONS } = require('./consolidation');
...
const OPEN_TRIGGER_REASON_SET = new Set(OPEN_TRIGGER_REASONS);
const STORED_TRIGGER_REASON_SET = new Set(STORED_TRIGGER_REASONS);
...
function normalizeTrigger(trigger, opts = {}) {
  assertPlainObject(trigger, 'trigger');
  const reasons = opts.allowLegacy ? STORED_TRIGGER_REASON_SET : OPEN_TRIGGER_REASON_SET;
  if (!reasons.has(trigger.reason)) throw new Error(`Invalid trigger reason: ${trigger.reason}`);
...
function validateRecord(record, expectedId = null) {
  ...
  // 退役理由不能再开新账，但历史账本必须一直可读。
  const normalizedTrigger = normalizeTrigger(record.trigger, { allowLegacy: true });
